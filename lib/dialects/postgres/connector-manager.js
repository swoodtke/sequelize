...
module.exports = (function() {
  ...
  ConnectorManager.prototype.query = function(sql, callee, options) {
    ...
    if (query.isTransactionStart(sql)) self.pendingTransactions += 1
    if (query.isTransactionEnd(sql)) self.pendingTransactions -= 1
  ...
  ConnectorManager.prototype.endQuery = function() {
    var self = this
    self.pendingQueries -= 1
    if (self.pendingQueries == 0 && self.pendingTransactions == 0) {
      setTimeout(function() {
        if (self.pendingQueries == 0 && self.pendingTransactions == 0) self.disconnect.call(self)
      }, 100)
    }
  }
